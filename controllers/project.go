...
// @Title DeleteAll
// @Description delete all user's projects
// @Param	userId		path 	int64	true		"The int you want to delete"
// @Success 200 {object} models.Response
// @Failure 403 :userId is invalid
// @router /:userId [delete]
func (this *ProjectController) DeleteAll() {
	var err error
	var response models.Response

	var userId int64
	userId, err = this.GetInt64(":userId")
	beego.Debug("DeleteAll", userId)
	if userId > 0 && err == nil {
		var svc service.ProjectService
		err = svc.DeleteAll(userId)
		if err == nil {
			response.Status = model.MSG_RESULTCODE_SUCCESS
			response.Reason = "success"
			response.Result = ""
